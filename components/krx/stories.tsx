...
import { Plus, X, Play, Heart, ChevronLeft, ChevronRight, Music, Trash2 } from "lucide-react";
...
export function Stories() {
  ...
  // All music: built-in + tracks from music library
  const allMusic = [
    ...BUILT_IN_MUSIC,
    ...tracks.map((t) => ({ title: t.title, artist: t.artist })),
  ];

  // Live stories: within 24 hours, not blocked
  const live = stories.filter(
    (s) => Date.now() - s.createdAt < 86_400_000 && !isBlocked(s.authorId)
  );

  const isLikedByMe = (s: Story) =>
    user ? (s.likedBy || []).includes(user.id) : false;
  ...
  return (
    <>
            ...
            <div key={story.id} className="relative flex flex-col items-center gap-1.5 flex-shrink-0">
              ...
              {isOwn && (
                <button
                  ...
                  className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-destructive rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow"
      ...
      {step === "preview" && pendingFile && (
        ...
        >
          <div
            className="bg-card rounded-2xl border border-border w-full max-w-sm overflow-hidden shadow-2xl"
          ...
          >
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                {selectedMusic && (
                  <span className="ml-auto text-xs text-primary font-medium">✓ выбрано</span>
                )}
              </div>
              <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                {allMusic.map((m, i) => (
                  <button
                    key={i}
                    onClick={() =>
                      setSelectedMusic(selectedMusic?.title === m.title ? null : m)
                    }
                    className={cn(
              ...
              <div className="flex gap-3 mt-4">
                <button
                  onClick={handleCancel}
                  className="flex-1 py-2.5 bg-muted text-foreground rounded-xl text-sm font-medium hover:bg-muted/80 transition-all"
                >
                  Отмена
                </button>
                <button
      ...
      {viewing && viewingIdx !== null && (
        ...
        >
            ...
            {live.map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i <= viewingIdx ? "bg-white" : "bg-white/30"
                )}
              />
            ))}
          </div>
          ...
          {/* Author info */}
          <div
            className="absolute top-8 left-4 flex items-center gap-3 z-10"
            onClick={(e) => e.stopPropagation()}
          >
          ...
          {/* Close */}
          <button
            className="absolute top-8 right-4 z-10 p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors"
            onClick={() => setViewingIdx(null)}
          >
          ...
          >
            ...
            {/* Music label */}
            {viewing.music && (
              <div className="absolute bottom-14 left-3 right-3 bg-black/60 backdrop-blur-sm rounded-xl px-3 py-2 flex items-center gap-2">
